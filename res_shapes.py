...
class shape_functions(object):
    """docstring for shape_functions."""

    def __init__(self, arg):
        super(shape_functions, self).__init__()
        self.arg = arg
...
class arc_shapes(object):
    """docstring for arc_shapes."""

    def __init__(self, arg):
        super(arc_shapes, self).__init__()
        self.arg = arg
    ...
    def arbarc(self,r, w, x0, y0, t0, tf, npoints=20):
        circ_arc = arc_shapes.circ_arc
        inner = circ_arc(r, x0, y0, theta0=t0, thetaf=tf, n=npoints)
        outer = circ_arc(r + w, x0, y0, theta0=t0, thetaf=tf, n=npoints)[::-1]
    ...
    def quarterarc_trench(self,r, width, gap, x0, y0, orient='NE', npoints=20):
        quarterarc = arc_shapes.quarterarc
    ...
    def solidarc(self,x0,y0,r,w_ind,o_gap,npoints=51):
        n=npoints
...
class rectangles(object):
    ...
    def straight_trench(self,l, gap, w, x0, y0, orientation='H'):
...
class DC_objects(object):
    """docstring for DC_objetcs."""

    def __init__(self, arg):
        super(DC_objects, self).__init__()
        self.arg = arg

    def DC_contacts(self,x0, y0, w, l, bond, contacts=4):
        rect = rectangles.rect
        x0 = float(x0)
        y0 = float(y0)
        LH = []
    ...
    def DC_contacts_etch(self,x0, y0, w, l, bond, gap, contacts=4):
        rect = rectangles.rect
        move = shape_functions.move
        x0 = float(x0)
        y0 = float(y0)
        shapes = []
...
class resonators(object):
    def __init__(self, arg):
        ...
        self.arg = arg
    def quarterwave(self,w, gap, nturns, lcap, lmeander, r, tail=True, ltail=100,
        ...
        '''
        straight_trench = rectangles.straight_trench
        rect = rectangles.rect
        halfarc_trench = arc_shapes.halfarc_trench
        halfarc = arc_shapes.halfarc
        quarterarc_trench = arc_shapes.quarterarc_trench
        quarterarc = arc_shapes.quarterarc
        thinning_trench = rectangles.thinning_trench
        move = shape_functions.move
        flip = shape_functions.flip
    ...
    def quarterwave_inverted(self,w, gap, nturns, lcap, lmeander, r, tail=True, ltail=100,
                    constriction=True, constrictionw=1, constrictionr=.5, spacer=5,
    ...
    def TRII(self,w_cap, w_in, l_in, gap):
        rect = rectangles.rect
        shapes = []
    ...
    def len_spiral(self,w, gap, l_straight, n):
        turns = np.arange(1, n+1, 1)
        turn_len = np.pi*(2*turns*(gap + w) - 3*w/2. + gap/2.)
        return 2*n*l_straight + np.sum(turn_len)

    def gavin_spiral(self,w, gap, l_straight, l_tot):
        rect = rectangles.rect
        halfarc = arc_shapes.halfarc
        len_spiral = resonators.len_spiral
        arbarc = arc_shapes.arbarc
        shapes = []
        n = 0
        l = 0
        while l < l_tot:
    ...
    def interdigital_capacitor(self,y0,w,l_arm,w_cap,gap,n):
        rect = rectangles.rect
        capacitor = []
    ...
    def opti_res(self,w_cap,l_cap_edge,gap,w_ind,r,n_turns):
        '''
    ...
    def interdig_optires(self,w,l_arm,w_cap,gap,n,r,w_ind,o_gap):
        ...
        design += [rect(w_cap,2*w_cap,(l_arm+gap+w_cap)/2 + gap/2,w_cap)]
        design += [rect((l_arm+gap+w_cap)/2 - gap/2,w_cap,0,3*w_cap)]
        design += [rect((l_arm+gap+w_cap)/2 - gap/2,w_cap,(l_arm+gap+w_cap)/2 + gap/2,3*w_cap)]
        ...
        '''
        rect = rectangles.rect
        solidarc = arc_shapes.solidarc
        interdigital_capacitor = resonators.interdigital_capacitor
...
class dots(object):
    ...
    def antidot_array(self,x0, y0, X, Y, w, s, n):
    ...
    def dot_line(self,x0, y0, L, w, s, orientation='V'):
        if orientation =='V':
    ...
    def dot_ring(self,x0, y0, gap, w, r):
...
class alignment(object):
    ...
    def alignment_mark(self,x0,y0,w,l):
        rect = rectangles.rect
        x0=float(x0)
        y0=float(y0)
        shapes = []
...
class misc(object):
    ...
    def feedline(self,cc, rat, r, W, H, bond, d_dots, straight=False):
        rect = rectangles.rect
        straight_trench = rectangles.straight_trench
        thinning_trench_style_2 = rectangles.thinning_trench_style_2
        quarterarc_trench = arc_shapes.quarterarc_trench
        quarterarc = arc_shapes.quarterarc
    ...
    def hw_sim(self,cc, gap, r, lcap, lstart, c_gap, npts=16):
        rect = rectangles.rect
        quarterarc = arc_shapes.quarterarc
        halfarc = arc_shapes.halfarc
    ...
    def capacitance_calc(self,y0,w,l_arm,w_cap,gap,n):
        rect = rectangles.rect
