...
for j in range(3):
    for k in range(3):
        ...
        res = [rs.move(i, -w_box/2 + w_cell/2 + k*w_cell - w[N]/2, w_box/2 - j*w_cell - w_cell/2 - max_yr/2) for i in res]
...
gdspy.write_gds('optires_3-5GHz.gds')#%gap, unit=1.0e-6, precision=1.0e-9)
